 ...
 *
 * 1. 最多同时设置两个菜单
 * 2. 菜单必须设置layout_gravity属性. start left end right
 */
public class SwipeItemLayout extends FrameLayout {
    ...
    /**
     * 某些情况下不能通过mIsOpen判断当前菜单是否开启或关闭，因为在调用 {@link #open()} 或 {@link #close()}
     * 时mIsOpen的值已经被改变，但此时ContentView还没有到达应该的位置。亦或者ContentView已经到拖拽达指定位置，
     * 但此时并没有松开手指，mIsOpen并不会重新赋值。
     */
     ...
     * 是否正在做开启动画
     */
    protected boolean isOpenAnimating() {
        if (mCurrentMenu != null) {
     ...
     * 是否正在做关闭动画
     */
    protected boolean isCloseAnimating() {
        if (mCurrentMenu != null) {
            int contentLeft = getContentView().getLeft();
